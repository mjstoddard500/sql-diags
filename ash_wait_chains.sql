...
/*
 ...
 Copyright: (c) http://blog.tanelpoder.com
     ...
     Usage: @ash_wait_chains <grouping_cols> <filters> <fromtime> <totime>

   Example: @ash_wait_chains username||':'||program2||event2 session_type='FOREGROUND' sysdate-1/24 sysdate

     Other: This script uses only the in-memory V$ACTIVE_SESSION_HISTORY, 
            use @dash_wait_chains.sql for accessiong the DBA_HIST_ACTIVE_SESS_HISTORY archive

  10g Note: Oracle 10g does not have the BLOCKING_INST_ID column in ASH so you'll need to
            comment out this column in this script. 
            This may give you somewhat incorrect results in RAC environment with global blockers.
*/
...
COLUMN wait_chain   FORMAT A300 WORD_WRAP
COLUMN "%This"      FORMAT A6
...
WITH bclass AS 
    (
        ...
        FROM v$waitstat),
    ash AS (
        SELECT /*+ INLINE QB_NAME(ash) LEADING(a) USE_HASH(u) SWAP_JOIN_INPUTS(u) */
                a.*,
                o.*,
                u.username,
                CASE 
                    WHEN a.session_type = 'BACKGROUND' AND a.program LIKE '%(DBW%)' 
                        THEN '(DBWn)'
                    WHEN a.session_type = 'BACKGROUND' OR REGEXP_LIKE(a.program, '.*\([PJ]\d+\)') 
                        THEN REGEXP_REPLACE(SUBSTR(a.program,INSTR(a.program,'(')), '\d', 'n')
                    ELSE
        ...
        FROM 
            ...
            ash_samples AS 
                (
                    SELECT /*+ INLINE */ DISTINCT sample_id FROM ash
                ),
            ash_data AS 
                (
                    SELECT /*+ INLINE */ * FROM ash
                ),
            chains AS 
                (
                    SELECT /*+ INLINE */ 
                            ...
                            REPLACE(SYS_CONNECT_BY_PATH(&1, '->'), '->', ' -> ')||
                                CASE 
                                    WHEN CONNECT_BY_ISLEAF = 1 AND d.blocking_session IS NOT NULL 
                                        THEN ' -> [idle blocker '||
                                            d.blocking_inst_id||','||
                                            d.blocking_session||','||
                                            ...
                                            (
                                                SELECT  ' ('||s.program||')' 
                                                FROM    gv$session s 
                                                WHERE   (s.inst_id, s.sid, s.serial#) 
                            ...
                            -- REPLACE(SYS_CONNECT_BY_PATH(&1, '->'), '->', ' -> ')||CASE WHEN CONNECT_BY_ISLEAF = 1 AND LEVEL > 1 THEN ' [sid='||session_id||' seq#='||TO_CHAR(seq#)||']' ELSE NULL END path,
                            -- there's a reason why I'm doing this 
                            -- REPLACE(SYS_CONNECT_BY_PATH(&1, '->'), '->', ' -> ') path,
                            -- there's a reason why I'm doing this (ORA-30004 :)
                            CASE 
